...
mmap(int length, int prot, int flags, struct file *file, int offset)
{
  ...
  for (uint64 i = 0; i < npages;)
  {
    ...
    for (int j = 0; j < 16; j++)
    {
      if (p->vmas[j].is_valid && (uint64)p->vmas[j].address <= addr && (uint64)p->vmas[j].address + p->vmas[j].length > addr)
      {
        vma_used = 1;
        break;
      };
...
int munmap(uint64 address, int length)
{
  struct proc *p = myproc();
  ...
  for (int i = 0; i < 16; i++)
  {
    if (p->vmas[i].is_valid && (uint64)p->vmas[i].address <= address && (uint64)p->vmas[i].address + p->vmas[i].length > address)
    {
      ...
      {
        if (walkaddr(p->pagetable, p->vmas[i].address + j * PGSIZE) != 0)
          all_unmapped = 0;
      }
